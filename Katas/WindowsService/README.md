# Kata 03_05 — Windows Service

**Library Kata** aus dem [CCD Coding Dojo](https://ccd-school.de/coding-dojo/) | [Original-Aufgabenstellung](https://ccd-school.de/coding-dojo/library-katas/windows-service/)

## Ziel

Eine Bibliothek entwickeln, die das Implementieren von Windows Services vereinfacht und Installation, Start, Stopp und Deinstallation ueber Kommandozeilenargumente ermoeglicht.

## Anforderungen

- `myservice /install` registriert den Service systemweit
- `net start myservice` startet ihn, `net stop myservice` stoppt ihn
- `myservice /deinstall` entfernt ihn wieder
- Entwickler implementieren das Interface `IService` mit Name, DisplayName, Description sowie `OnStart` und `OnStop`
- `EasyService.Run()` interpretiert die Kommandozeilenargumente

## Beispiele und Testfaelle

- Hinweis fuer .NET 10: `Microsoft.Extensions.Hosting.WindowsServices` mit `BackgroundService` ist die moderne Entsprechung -- vergleiche deine Loesung damit

## Variationen und Randbedingungen

- Alternative ohne Interface: Parameter als Strings und Lambdas (`Action`) uebergeben statt `IService` zu implementieren
- Kommando `/run` fuer einen Testlauf ohne Installation, um debuggen zu koennen
